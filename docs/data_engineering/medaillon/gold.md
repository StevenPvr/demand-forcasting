...
- dense
- temporalement safe
- exploitable par un pipeline TFT robuste
- structure pour l'entrainement, l'optimisation et l'evaluation

La `gold` n'est pas une couche de reporting.
Elle est la couche prete pour le modele du wedge actuel Praedixa :
...
L'objectif de la `gold` V1 est de tester si un modele entraine sur des jeux de donnees de demande multi-sources peut generaliser sur `bakery`.

Le cadre d'evaluation est donc :

- `train` sur les jeux de donnees hors `bakery`
- `val` sur les jeux de donnees hors `bakery`
...
- `bronze`
  - projection panel proche source
- `silver`
...
Pour un pipeline TFT, cette densification est plus propre qu'un panel sparse, car elle rend les :
...
## Colonnes de contexte et d'hypothese client
...
- trop lourds
- plus fragiles
- peu utiles pour un premier test de generalisation TFT `D+1`
...
Les plus gros datasets de la `gold` dominent fortement `bakery` en volume.

La `gold` doit donc porter des colonnes de ponderation afin d'eviter qu'un modele global n'apprenne quasi exclusivement les sources les plus volumineuses.
...
- D+1
- zero-shot `bakery`
- structure pour un modele TFT exploitable
