...
def _uses_legacy_target_scaler(fitted_model: FittedTFTModel) -> bool:
    ...
    return bool(strategy.get("legacy_external_scaler")) or str(strategy.get("kind")) == "legacy_external_standard_scaler"


def _inverse_target_scaler(values: np.ndarray, *, fitted_model: FittedTFTModel) -> np.ndarray:
    if fitted_model.target_scaler is None or values.size == 0 or not _uses_legacy_target_scaler(fitted_model):
