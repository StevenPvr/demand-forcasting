...
from praedixa.demand_forecast.training.feature_audit import (
...
from praedixa.demand_forecast.training.sampling import log_sampling_summary, resolve_sampling_store_col
...
def _load_parquet_frames(
    *,
    ...
    return LoadedOptimisationFrames(*loaded, Path(train_input_path), Path(tuning_input_path))
...
def _normalize_frames(*, train_frame: pd.DataFrame, tuning_frame: pd.DataFrame, date_col: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return train_frame.sort_values(date_col).reset_index(drop=True), tuning_frame.sort_values(date_col).reset_index(drop=True)
...
def _resolve_target_context(
    *,
    ...
    target_contract = resolve_target_contract(loaded.train_frame, loaded.tuning_frame, requested_target_col=target_col)
    ...
    sample_store_col = resolve_sampling_store_col(train_frame) if loaded.train_path and loaded.tuning_path else loaded.sample_store_col
    return train_frame, tuning_frame, sample_store_col, target_contract, resolved_target_col, target_contract.absolute_target_col
...
def _resolve_feature_context(
    *,
    ...
    raw_feature_cols = select_tft_feature_columns(
        train_frame,
        excluded_cols={date_col, resolved_target_col, absolute_target_col, *DEFAULT_EXCLUDED_RISKY_FEATURE_COLS},
    )
    feature_cols, constant_feature_cols = drop_constant_feature_columns(train_frame, raw_feature_cols)
    identifier_feature_cols = [col for col in DEFAULT_IDENTIFIER_FEATURE_COLS if col in train_frame.columns and col not in feature_cols]
    return feature_cols, raw_feature_cols, constant_feature_cols, identifier_feature_cols
...
def prepare_optimisation_context(
    *,
    ...
    train_frame, tuning_frame = _normalize_frames(train_frame=loaded.train_frame, tuning_frame=loaded.tuning_frame, date_col=date_col)
    ...
    train_frame, tuning_frame, sample_store_col, target_contract, resolved_target_col, absolute_target_col = _resolve_target_context(
        loaded=normalized_loaded,
        target_col=target_col,
        logger=logger,
    )
    feature_cols, raw_feature_cols, constant_feature_cols, identifier_feature_cols = _resolve_feature_context(
        train_frame=train_frame,
        date_col=date_col,
...
def _dataset_weight_summaries(context: PreparedOptimisationContext) -> tuple[dict[str, dict[str, float | int]], dict[str, dict[str, float | int]]]:
    return (
        summarize_dataset_weights(context.train_frame, dataset_source_col=DEFAULT_DATASET_SOURCE_COL),
        summarize_dataset_weights(context.tuning_frame, dataset_source_col=DEFAULT_DATASET_SOURCE_COL),
    )
...
def log_optimisation_context(
    *,
    ...
    logger.info("Target contract for optimisation: %s", build_target_contract_metadata(context.target_contract))
    logger.info("Identifier features included in optimisation: %s", context.identifier_feature_cols)
    logger.info("Constant features dropped before optimisation: %s", context.constant_feature_cols)
...
def _persist_outputs(
    *,
    ...
    _json_dump(output_paths["baseline_report"], {"best_baseline": artifacts.best_baseline, "all_baselines": artifacts.baseline_rows})
    _json_dump(
...
def _baseline_and_hpo_outputs(
    *,
    ...
    resolved_target_transform: str,
    logger: logging.Logger,
    optimize_fn: Callable[..., tuple[dict[str, object], pd.DataFrame, dict[str, object]]],
    ...
    return baseline_folds, training_folds, baseline_rows, best_baseline, best_params, tuning_report, hpo_runtime_metadata
...
def run_optimisation_pipeline(
    *,
    ...
    baseline_folds, training_folds, baseline_rows, best_baseline, best_params, tuning_report, hpo_runtime_metadata = _baseline_and_hpo_outputs(
        context=context,
    ...
    logger.info("Optimisation artifacts written: best_params=%s trials_report=%s", output_paths["best_params"], output_paths["optuna_trials_report"])
    return output_paths
