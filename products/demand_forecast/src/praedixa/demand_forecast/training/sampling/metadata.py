...
_PROJECTED_TFT_FEATURE_ROLES: frozenset[str] = frozenset(
    {
        "static_categorical",
        "static_real",
...
def resolve_gold_projection_columns(
    ...
    *,
    ...
    for column in schema_preview.columns:
        if _is_numeric_or_bool_projection_column(
            schema_preview[column]
        ) or _is_explicit_model_feature_projection_column(column):
...
def _is_explicit_model_feature_projection_column(column: str) -> bool:
    return TFT_EXPLICIT_ROLE_BY_COLUMN.get(column) in _PROJECTED_TFT_FEATURE_ROLES
