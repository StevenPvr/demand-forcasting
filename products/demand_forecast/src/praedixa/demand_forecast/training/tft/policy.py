...
def should_serialize_local_cpu_folds() -> bool:
    from praedixa.demand_forecast.backends.tft.model_common import (
        should_serialize_local_cpu_folds as resolve_should_serialize,
    )

    return bool(resolve_should_serialize())
...
def _anchored_log_bounds(anchor: object, *, low: float, high: float) -> tuple[float, float]:
    if not isinstance(anchor, (int, float)) or float(anchor) <= 0.0:
        return low, high
...
def sample_optuna_params(
    ...
    *,
    ...
    if stage_name == "stage_b":
        ...
        broad_learning_rate_low, broad_learning_rate_high = (
            _batch_scaled_learning_rate_bounds(batch_size)
        )
        learning_rate_low, learning_rate_high = _anchored_log_bounds(
            anchor_params.get("learning_rate") if anchor_params else None,
            low=broad_learning_rate_low,
            high=broad_learning_rate_high,
        )
        ...
        weight_decay_low, weight_decay_high = _anchored_log_bounds(
    ...
    *, execution_policy: dict[str, object]
    ...
    if callable(is_available) and bool(is_available()):
        ...
        metadata["device_count"] = (
            int(cast(Any, device_count())) if callable(device_count) else None
        )
        metadata["gpu_name"] = (
            str(cast(Any, get_device_name(0))) if callable(get_device_name) else None
        )
        metadata["cuda_version"] = getattr(version, "cuda", None)
    return metadata
    ...
    *,
    ...
    return {
        ...
        "best_improvement_pct": (
            resolved_best_trial.user_attrs.get("baseline_wape_improvement_pct")
        ),
