...
def _guardrail_failure_reason(
    *,
    ...
    collapsed = [
        ...
        if float(score)
        > (baseline_wape * DEFAULT_TUNING_GUARDRAIL_DATASET_WAPE_COLLAPSE_MULTIPLIER)
    ]
    if collapsed:
...
def _best_observed_trial(
    ...
    if not candidates:
        ...
        raise RuntimeError(
            "Optuna search completed without any usable scored trials. "
            f"Failure reasons: {failure_reasons}"
        )
    return min(
        candidates,
        key=lambda trial: float(trial.user_attrs["mean_wape"]),
    )
