...
from praedixa.demand_forecast.training.config.constants import DEFAULT_DATASET_SOURCE_COL, DEFAULT_EXCLUDED_RISKY_FEATURE_COLS
...
def drop_constant_feature_columns(
    ...
    if tuning_frame is None:
        return list(feature_cols), []
    zero_only_cols = [
        column
        for column in feature_cols
        if _is_zero_only_feature(train_frame, tuning_frame, column)
    ]
    zero_only = set(zero_only_cols)
    return [column for column in feature_cols if column not in zero_only], zero_only_cols


def _is_zero_only_feature(
    train_frame: pd.DataFrame,
    tuning_frame: pd.DataFrame,
    column: str,
) -> bool:
    if column not in train_frame.columns or column not in tuning_frame.columns:
        return False
    return _is_zero_only_series(train_frame[column]) and _is_zero_only_series(
        tuning_frame[column]
    )
...
def _is_zero_only_series(series: pd.Series) -> bool:
    ...
    if pd.api.types.is_bool_dtype(series):
        return bool((~series.astype(bool)).all())
    if pd.api.types.is_numeric_dtype(series):
        return bool(series.eq(0).all())
    zero_like_values = {"0", "0.0"}
    normalized = series.astype("string").str.strip()
    return bool(normalized.isin(zero_like_values).all())
