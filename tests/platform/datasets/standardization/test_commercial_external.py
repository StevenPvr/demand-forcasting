...
from praedixa.platform.datasets.standardization.commercial_external import (  # noqa: E402
    build_commercial_external_standardized_dataset,
    commercial_dataset_compatibility_matrix,
...
class GlobalDatasetCommercialTests(unittest.TestCase):
    ...
    def test_build_commercial_external_dataset_accepts_first_party_and_synthetic_canonical_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ...
            synthetic = pl.DataFrame(
                {
                    "dt": [pd.Timestamp("2024-01-02").date()],
                    "location_id": ["site_2"],
                    "product_id": ["sku_2"],
                    "observed_demand_qty": [8.0],
                }
            )
            first_party.write_parquet(tmp_dir / "first_party_daily.parquet")
            synthetic.write_parquet(tmp_dir / "synthetic_v1.parquet")

            output_path = tmp_dir / "commercial_external_daily.parquet"
            result_path = build_commercial_external_standardized_dataset(
            ...
            self.assertEqual([record["dataset_source"] for record in records], ["first_party_daily", "synthetic_v1"])
            self.assertEqual(records[0]["series_id"], "site_1__sku_1")
            self.assertEqual(records[1]["series_id"], "site_2__sku_2")
