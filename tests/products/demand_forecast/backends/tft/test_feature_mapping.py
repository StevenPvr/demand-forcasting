...
class TFTFeatureMappingTests(unittest.TestCase):
    ...
    def test_roles_cover_representative_columns(self) -> None:
        ...
        self.assertEqual(
            TFT_EXPLICIT_ROLE_BY_COLUMN["rolling_mean_7"], "time_varying_known_real"
        )
    ...
    def test_feature_contract_marks_known_features_as_available(self) -> None:
        ...
        self.assertEqual(
            contract["target_holiday_flag"]["decision_time_scope"],
            "target_calendar_known",
        )
        self.assertEqual(
            contract["rolling_mean_7"]["max_publication_lag_hours"],
            0,
        )
        self.assertEqual(contract["rolling_mean_7"]["leakage_risk_level"], "low")

    def test_validate_feature_contract_returns_contract_for_valid_mapping(self) -> None:
        contract = validate_feature_contract(
            ["location_id", "product_family", "target_holiday_flag"]
        )
