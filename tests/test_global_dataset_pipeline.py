...
from research_praedixa.global_dataset.pipeline import build_global_daily_standardization  # noqa: E402
...
def _write_fixture_parquet(path: Path, dataset_source: str) -> None:
    ...
    elif dataset_source == "commercial_external":
        frame = {
...
class GlobalDatasetPipelineTests(unittest.TestCase):
    def test_build_global_daily_standardization_writes_manifest_from_source_summaries(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch(
                "research_praedixa.global_dataset.pipeline.build_freshretail_standardized_dataset",
                side_effect=lambda output_path: _copy_fixture(fresh_path, output_path),
            ):
                with mock.patch(
                    "research_praedixa.global_dataset.pipeline.build_commercial_external_standardized_dataset",
                    side_effect=lambda output_path, raw_dir: _copy_fixture(commercial_path, output_path),
                ):
