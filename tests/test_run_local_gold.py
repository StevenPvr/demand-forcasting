...
from scripts.run_local_gold import (  # noqa: E402
...
class RunLocalGoldTests(unittest.TestCase):
    def test_run_local_gold_script_can_be_called_directly_via_absolute_path(self) -> None:
        result = subprocess.run(
            [sys.executable, str(PROJECT_ROOT / "scripts" / "run_local_gold.py"), "--help"],
            capture_output=True,
            text=True,
            cwd=PROJECT_ROOT,
            check=False,
    ...
    def test_run_local_gold_calls_dbt_run_and_test(self) -> None:
        ...
        with (
            mock.patch("scripts.run_local_gold.refresh_open_exogenous_inputs", return_value={"fetch": {}, "bronze_load": {}}),
            mock.patch("scripts.run_local_gold.resolve_dbt_executable", return_value=".venv/bin/dbt"),
            mock.patch("scripts.run_local_gold.ensure_dbt_profiles_file", return_value=PROJECT_ROOT / "warehouse" / "profiles.yml"),
            mock.patch("scripts.run_local_gold.dbt_packages_installed", return_value=True),
            mock.patch("scripts.run_local_gold.run_dbt_command") as run_dbt_command_mock,
        ):
            result = run_local_gold(config)

        self.assertEqual(run_dbt_command_mock.call_count, 2)
        self.assertEqual(run_dbt_command_mock.call_args_list[0].args[1], "run")
        self.assertEqual(run_dbt_command_mock.call_args_list[0].kwargs["select"], "+tag:gold")
        self.assertEqual(run_dbt_command_mock.call_args_list[1].args[1], "test")
    ...
    def test_run_local_gold_can_skip_dbt_tests(self) -> None:
        ...
        self.assertEqual(run_dbt_command_mock.call_count, 1)
        self.assertEqual(run_dbt_command_mock.call_args_list[0].args[1], "run")
        self.assertTrue(result["dbt_run"])
    ...
    def test_run_local_gold_bootstraps_dbt_packages_when_missing(self) -> None:
        ...
        with (
            ...
            mock.patch("scripts.run_local_gold.resolve_dbt_executable", return_value=".venv/bin/dbt"),
            mock.patch("scripts.run_local_gold.ensure_dbt_profiles_file", return_value=PROJECT_ROOT / "warehouse" / "profiles.yml"),
            mock.patch("scripts.run_local_gold.dbt_packages_installed", return_value=False),
            mock.patch("scripts.run_local_gold.run_dbt_command") as run_dbt_command_mock,
        ):
    ...
    def test_run_local_gold_refreshes_open_exogenous_by_default(self) -> None:
        ...
        with (
            mock.patch("scripts.run_local_gold.refresh_open_exogenous_inputs", return_value={"fetch": {}, "bronze_load": {}}) as refresh_mock,
            mock.patch("scripts.run_local_gold.resolve_dbt_executable", return_value=".venv/bin/dbt"),
            mock.patch("scripts.run_local_gold.ensure_dbt_profiles_file", return_value=PROJECT_ROOT / "warehouse" / "profiles.yml"),
            mock.patch("scripts.run_local_gold.dbt_packages_installed", return_value=True),
            mock.patch("scripts.run_local_gold.run_dbt_command"),
        ):
            result = run_local_gold(config)
    ...
    def test_refresh_open_exogenous_inputs_fetches_and_loads_matching_specs(self) -> None:
        ...
        with (
            mock.patch("scripts.run_local_gold.fetch_open_exogenous_data", return_value={"manifest_path": Path("data/external_open/open_exogenous_manifest.json")}) as fetch_mock,
            mock.patch("scripts.run_local_gold.build_open_exogenous_runtime_config_from_env", return_value=mock.sentinel.runtime_config),
            mock.patch("scripts.run_local_gold.default_open_exogenous_bronze_specs", return_value=["spec_a", "spec_b"]) as specs_mock,
            mock.patch("scripts.run_local_gold.load_selected_bronze_specs", return_value={"local_warehouse_row_counts": {"open_weather_daily": 123}}) as load_mock,
        ):
