...
class RunLocalSilverTests(unittest.TestCase):
    def test_run_local_silver_script_can_be_called_directly_via_absolute_path(self) -> None:
        result = subprocess.run(
            [sys.executable, str(PROJECT_ROOT / "scripts" / "run_local_silver.py"), "--help"],
            capture_output=True,
            text=True,
            cwd=PROJECT_ROOT,
            check=False,
    ...
    def test_run_local_silver_calls_bronze_loader_then_dbt_run_and_test(self) -> None:
        ...
        with (
            mock.patch(
                "scripts.run_local_silver.build_commercial_external_standardized_dataset",
                return_value=PROJECT_ROOT / "data" / "global_dataset" / "commercial_external_daily.parquet",
            ) as commercial_mock,
            mock.patch("scripts.run_local_silver.default_active_bronze_specs", return_value=["active_specs"]) as specs_mock,
            mock.patch("scripts.run_local_silver.load_selected_bronze_specs", return_value={"ok": True}) as load_mock,
            mock.patch("scripts.run_local_silver.resolve_dbt_executable", return_value=".venv/bin/dbt"),
            mock.patch("scripts.run_local_silver.ensure_dbt_profiles_file", return_value=PROJECT_ROOT / "warehouse" / "profiles.yml"),
            mock.patch("scripts.run_local_silver.dbt_packages_installed", return_value=True),
            mock.patch("scripts.run_local_silver.run_subprocess") as run_mock,
        ):
        ...
        self.assertEqual(second_command[-1], "+tag:silver")
    ...
    def test_run_local_silver_can_skip_bronze_reload_and_dbt_tests(self) -> None:
        ...
        with (
            mock.patch("scripts.run_local_silver.build_commercial_external_standardized_dataset") as commercial_mock,
            mock.patch("scripts.run_local_silver.load_selected_bronze_specs") as load_mock,
            mock.patch("scripts.run_local_silver.resolve_dbt_executable", return_value=".venv/bin/dbt"),
            mock.patch("scripts.run_local_silver.ensure_dbt_profiles_file", return_value=PROJECT_ROOT / "warehouse" / "profiles.yml"),
            mock.patch("scripts.run_local_silver.dbt_packages_installed", return_value=True),
            mock.patch("scripts.run_local_silver.run_subprocess") as run_mock,
    ...
    def test_ensure_dbt_profiles_file_uses_existing_profile(self) -> None:
        ...
        self.assertEqual(profile_path, PROJECT_ROOT / "warehouse" / "profiles.yml")
        self.assertTrue(profile_path.exists())

    def test_dbt_packages_installed_reads_expected_location(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "warehouse" / "dbt_packages").mkdir(parents=True)
    ...
    def test_run_local_silver_bootstraps_dbt_packages_when_missing(self) -> None:
        ...
        with (
            ...
            mock.patch("scripts.run_local_silver.resolve_dbt_executable", return_value=".venv/bin/dbt"),
            mock.patch("scripts.run_local_silver.ensure_dbt_profiles_file", return_value=PROJECT_ROOT / "warehouse" / "profiles.yml"),
            mock.patch("scripts.run_local_silver.dbt_packages_installed", return_value=False),
            mock.patch("scripts.run_local_silver.run_subprocess") as run_mock,
        ):
        ...
        self.assertEqual(run_mock.call_args_list[1].args[0][-1], "+tag:silver")
