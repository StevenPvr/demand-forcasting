...
- `PRAEDIXA_LOCAL_BACKUP_DIR=data/local_backup`
...
- le runner local bootstrap automatiquement `warehouse/profiles.yml` si besoin
...
- macro mensuelle / trimestrielle `as-of` : `FRED` public CSV endpoint
...
```bash
.venv/bin/python scripts/load_bronze_duckdb.py
```

2. Lancer la silver avec la cible locale par defaut :

```bash
dbt run --project-dir warehouse --profiles-dir warehouse --select +tag:silver
```

3. Valider :

```bash
dbt test --project-dir warehouse --profiles-dir warehouse --select +tag:silver
```
...
```bash
dbt run --project-dir warehouse --profiles-dir warehouse --select +tag:gold
dbt test --project-dir warehouse --profiles-dir warehouse --select tag:gold
```
